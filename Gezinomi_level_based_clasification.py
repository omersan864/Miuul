...
df = pd.read_excel("Numpy-Pandas_Learn/datasets/miuul_gezinomi.xlsx")
df.head()
df.shape
df.info()
...
category = ["Last Minuters", "Potential Planners", "Planners", "Early Bookers"]

df["EB_Score"] = pd.cut(df["SaleCheckInDayDiff"], bins=[-1, 7, 30, 90, 650], labels=category)
...
df.groupby(["SaleCityName", "EB_Score"]).agg({"Price":["mean", "count"]})
...
df.groupby(["ConceptName", "CInDay"]).agg({"Price": "mean"})
...
new_user = "AYDIN_YARIM PANSIYON_LOW"
