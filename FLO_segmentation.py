...
df_ = pd.read_csv('CRM_analytic/datasets/flo_data_20k.csv')
df = df_.copy()
...
target_segments_customer_ids = rfm[rfm["segment"].isin["champions","loyal_customers"]]
cuts_ids  = df[(df["master_id"].isin("target_segments_customers_ids")) & (df["interested_in_categoris"].str.contains("KADIN"))]
cuts_ids.to_csv("new_target_customers_id", index=False)
